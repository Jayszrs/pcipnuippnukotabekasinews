...
export const FloatingWidgets = () => {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-[1000] flex flex-col gap-3 items-end">
      ...
      {isOpen && (
        <div className="w-[320px] sm:w-[380px] h-[480px] sm:h-[550px] bg-card text-card-foreground rounded-[2.5rem] border border-border shadow-2xl shadow-black/10 dark:shadow-black/40 flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500 relative">
          
          {/* Header Chatbox */}
          <div className="gradient-primary text-primary-foreground p-5 flex items-center justify-between border-b border-white/10 relative">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-muted/35 no-scrollbar">
            {messages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`flex gap-2.5 ${isAi ? "justify-start" : "justify-end"}`}>
                  ...
                  <div className={`p-4 rounded-3xl max-w-[80%] text-xs leading-relaxed font-semibold shadow-sm border ${
                    ...
                    <span className={`text-[8px] font-bold block text-right mt-1.5 ${isAi ? "text-muted-foreground" : "text-primary-foreground/70"}`}>
            ...
            {isThinking && (
              <div className="flex gap-2.5 justify-start">
                ...
                <div className="p-3.5 bg-card rounded-3xl rounded-tl-none border border-border shadow-sm flex items-center gap-1.5">
          ...
          {/* Quick reply chips */}
          <div className="px-5 py-2.5 bg-card border-t border-border flex gap-2 overflow-x-auto no-scrollbar shrink-0">
            <button onClick={() => handleSendMessage("Sejarah IPNU IPPNU")} className="px-3 py-1.5 bg-muted hover:bg-primary/10 hover:text-primary rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 transition-colors border border-border">Sejarah 🌿</button>
            <button onClick={() => handleSendMessage("Cara Gabung Anggota")} className="px-3 py-1.5 bg-muted hover:bg-primary/10 hover:text-primary rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 transition-colors border border-border">Gabung ✨</button>
            <button onClick={() => handleSendMessage("Ajarin saya koding React.js")} className="px-3 py-1.5 bg-muted hover:bg-primary/10 hover:text-primary rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 transition-colors border border-border">Belajar Coding 💻</button>
            <button onClick={() => handleSendMessage("Bantu ngerjain tugas sekolah")} className="px-3 py-1.5 bg-muted hover:bg-primary/10 hover:text-primary rounded-full text-[10px] font-black uppercase tracking-wider shrink-0 transition-colors border border-border">Tugas Sekolah 📚</button>
          </div>

          {/* Input Chat Box */}
          <div className="p-4 bg-card border-t border-border flex gap-2 items-center shrink-0">
            <input
              type="text"
              value={inputText}
              ...
              className="flex-1 px-4 py-3 bg-muted rounded-2xl text-xs font-bold border border-transparent outline-none focus:bg-background focus:ring-1 focus:ring-primary transition-all placeholder:text-muted-foreground"
            />
            <button
              onClick={() => handleSendMessage()}
              disabled={!inputText.trim()}
              className="p-3 bg-primary hover:bg-primary-deep text-primary-foreground rounded-2xl disabled:opacity-40 transition-all shadow-md active:scale-95 shrink-0"
            >
      ...
      <div className="flex flex-col gap-3 items-end">
        ...
        <button 
          ...
          className="h-14 w-14 rounded-full gradient-primary text-primary-foreground shadow-2xl flex items-center justify-center hover:scale-110 active:scale-95 transition-transform duration-300 border border-primary/30 group relative"
        >
          {isOpen ? (
            <X className="h-6 w-6" />
          ) : (
