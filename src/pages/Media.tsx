...
const Media = () => {
  const { articles, getVideos } = useArticles();
        ...
        {tab === "foto" ? (
          articles.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {articles.map((a, i) => (
                <Link to={`/berita/${a.slug}`} key={a.id} className={`relative overflow-hidden rounded-sm group cursor-pointer ${i % 5 === 0 ? "md:col-span-2 md:row-span-2 aspect-square" : "aspect-square"}`}>
          ...
          videos.length === 0 ? (
            <p className="text-muted-foreground">Belum ada video. Admin dapat menambahkan video dari halaman tulis berita.</p>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {videos.map((a) => (
                <Link to={`/berita/${a.slug}`} key={a.id} className="group cursor-pointer">
